...
const logoutMainButton = document.getElementById('logoutMainButton');
...
const cancelSyncButton = document.getElementById('cancelSyncButton');
  ...
  function toggleTheme() {
    ...
    trackThemeChange(currentTheme); // Track theme usage
  ...
  async function login(password) {
    try {
      ...
      if (data.success) {
        ...
        article.innerHTML = `
          <div style="text-align: center; padding: 60px 20px; color: var(--text-muted);">
            <h2 style="margin: 0 0 16px 0; font-size: 24px;">Welcome to Senior Dev Cheatsheet</h2>
            <p style="margin: 0 0 24px 0; font-size: 16px;">You're logged in as admin. Select a technology from the sidebar to get started.</p>
            <p style="margin: 0; font-size: 14px; color: var(--text-secondary);">
              Click the edit button to modify any cheatsheet.
            </p>
          </div>
        `;
      ...
      showMessage('Login failed. Make sure the server is running.', 'error');
      return false;
    }
  }
  ...
  function installEditMode() {
    ...
    logoutMainButton.addEventListener('click', logout);
